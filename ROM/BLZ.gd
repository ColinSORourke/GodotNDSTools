...
static func Decompress(buffer: PackedByteArray) -> PackedByteArray:
	#var reader: FileAccess = FileAccess.open(file, FileAccess.READ)
	
	var appendedDataAmt: int = detectAppendedData(buffer)
	var ptr: int = 0
	
	if (appendedDataAmt == -1):
		return []
	
	var dataSize: int = buffer.size()
	var appendedData: PackedByteArray
	if (appendedDataAmt == 0):
		appendedData = PackedByteArray()
	else:
		appendedData = buffer.slice(ptr, dataSize - appendedDataAmt)
		ptr = dataSize - appendedDataAmt
		dataSize -= appendedDataAmt
	
	ptr = dataSize - 4
	if (buffer.decode_u32(ptr) == 0):
		return []
	ptr += 4
	
	ptr = buffer.size() - 8
	var composite: int = buffer.decode_u32(ptr)
	ptr += 4
	var headerLength: int = composite >> 24
	var compressedLength: int = composite & 0xFFFFFF
	var extraSize: int = buffer.decode_u32(ptr)
	ptr += 4
	...
	ptr = dataSize - headerLength
	while (ptr < buffer.size() -8):
		var byte: int = buffer.decode_u32(ptr)
		ptr += 4
		if (byte & 0xFF != 0xFF):
	...
	ptr = 0
	var passThruData: PackedByteArray = buffer.slice(ptr, passThruLength) 
	ptr += passThruLength
	
	var compressedData: PackedByteArray = buffer.slice(ptr, compressedLength - headerLength)
	ptr += compressedLength - headerLength
	...
	passThruData.append_array(decompressedData)
	passThruData.append_array(appendedData)
	return passThruData
	
# Algorithm taken from NDSPY/CodeCompression
static func Compress(buffer: PackedByteArray, arm9: bool = false) -> PackedByteArray:
	...
	if (not compressedBuff or (buffer.size() - 4 < ( (compressedBuff.size() + 3) & ~4)  + 8) ):
		...
		prefix.append_array(compressedBuff)
		return prefix

static func checkCompressed(file: PackedByteArray) -> bool:
	var appendedDataAmt: int = detectAppendedData(file)
	if (appendedDataAmt == -1):
		return false
	return true
...
static func detectAppendedData(file: PackedByteArray) -> int:
	var possibleAmt: int = 0
	var ptr: int = 0
	
	while (possibleAmt < 0x20):
		if (file.size() - possibleAmt - 5 > 0):
			var headerLength: int = file.size() - possibleAmt - 5
			ptr = file.size() - possibleAmt - 8
			var composite: int = file.decode_u32(ptr)
			ptr += 4
			headerLength = composite >> 24
			var compressedLength: int = composite & 0xFFFFFF
			
			if (headerLength < 8 or compressedLength > file.size()):
				possibleAmt += 4
				# end here
			else: 
				ptr = file.size() - possibleAmt - headerLength
				var invalidFound: bool = false
				while (!invalidFound && ptr < file.size() - possibleAmt - 8):
					var byte: int = file.decode_u32(ptr)
					ptr += 4
					if (byte & 0xFF != 0xFF):
						invalidFound = true
						ptr = file.size() - 1
